...
class Graph
{
    ...
    /**
     * Build a basic graph containing only information from the graph and edges database tables
     */
...
class UserGraph extends Graph
{
    ...
    function __construct($userID)
    {
        ...
        $q = "SELECT nodeID, score FROM ike_genrescore WHERE userID = :uid";
        $mq = $db->prepare($q);
        $mq->bindParam(':uid', $userID, PDO::PARAM_INT);
        $mq->execute();
        ...
        while ($mq->fetch()) {
            ...
            $node->changeValue($node->getValue() * $changes[$score]);
        }
        ...
        $list = array();
        while ($mq->fetch()) {
            //Apply user rating to a node and its edges
    ...
    /**
     * Returns 10 nodes with highest ratings in a MST built from the node with the highest user-rating
     */
    ...
    /**
     * Specific function which searches in a returnList for a certain node
     */
