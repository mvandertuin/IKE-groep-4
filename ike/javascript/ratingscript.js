function onl() {
    ...
    $(".votedon").off("click");
    $(".votedon").click(function () {
        ouralert("U heeft deze rating al uitgebracht op deze plaat, u kunt hem nog wel wijzigen.")
    });
    $(".delete").click(del);
    $("#alertbox").overlay({
        mask:{
...
function handle(event) {
    ...
    $.post("addrating/", { mbid:event.target.id.replace("_" + rat, ""), rhash:"2pm", albumname:"geniasal", rat:rat}, function (data) {
        change(data, event)
    });
...
function change(data, event) {
    ...
    $(event.target).addClass("votedon").off("click").click(function () {
        ouralert("U heeft deze rating al uitgebracht op deze plaat, u kunt hem nog wel wijzigen.")
    });
    ouralert(data);
